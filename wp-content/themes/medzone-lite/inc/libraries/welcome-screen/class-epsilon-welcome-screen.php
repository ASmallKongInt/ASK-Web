...
class Epsilon_Welcome_Screen {
	...
	public function __construct( $config = array() ) {
		...
		$upsells   = get_option( 'medzone_lite_theme_upsells', false );
		$litevspro = get_option( 'medzone_lite_lite_vs_pro', false );
		if ( $upsells ) {
	...
	public function add_privacy_options() {
		$options = array(
			'medzone_lite_recommended_actions' => array(
				'id'      => 'medzone_lite_recommended_actions',
				'value'   => true,
				'label'   => __( 'Customizer Recommended Actions', 'epsilon-framework' ),
				'checked' => get_option( 'medzone_lite_recommended_actions', false ),
			),
		);

		if ( ! $this->edd ) {
			$options['medzone_lite_theme_upsells'] = array(
				'id'      => 'medzone_lite_theme_upsells',
				'value'   => true,
				'label'   => __( 'Theme Upsells', 'epsilon-framework' ),
				'checked' => get_option( 'medzone_lite_theme_upsells', false ),
			);

			$options['medzone_lite_lite_vs_pro'] = array(
				'id'      => 'medzone_lite_lite_vs_pro',
				'value'   => true,
				'label'   => __( 'Lite VS Pro Table', 'epsilon-framework' ),
				'checked' => get_option( 'medzone_lite_lite_vs_pro', false ),
			);
		}
	...
	private function get_actions_left() {
		if ( ! empty( $this->actions ) ) {
			$defaults = array(
				'medzone-check-cf7'   => true,
				'medzone-import-data' => true,
			);
	...
	private function set_default_sections( $config = array() ) {
		$arr = array(
			...
			'features'            => array(
				...
				'path'  => get_template_directory() . '/inc/libraries/welcome-screen/sections/features.php',
			),
			'privacy'             => array(
				'id'    => 'privacy',
		...
		$removed_table = get_option( 'medzone_lite_lite_vs_pro', false );
		if ( $removed_table ) {
			unset( $arr['features'] );
		}
